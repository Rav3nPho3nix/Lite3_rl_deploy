...
// ---------------------------------------------------------------------------
struct Lite3FlatPolicyRunnerONNX::OrtImpl {
    Ort::Env env;
...
// ---------------------------------------------------------------------------
Lite3FlatPolicyRunnerONNX::Lite3FlatPolicyRunnerONNX(std::string policy_name)
    ...
    // model_path_ = GetAbsPath() + "/../policy/ppo/lite3_flat_policy.onnx";
    // model_path_ = GetAbsPath() + "/../policy/ppo/policy.onnx";
    model_path_ = GetAbsPath() + "/../policy/ppo/lite3_flat_policy_2.onnx";
...
Lite3FlatPolicyRunnerONNX::~Lite3FlatPolicyRunnerONNX() = default;

// ---------------------------------------------------------------------------
// Public interface
// ---------------------------------------------------------------------------
void Lite3FlatPolicyRunnerONNX::DisplayPolicyInfo() {
    std::cout << "ONNX policy: " << policy_name_ << "\n";
    std::cout << "path: " << model_path_ << "\n";
    std::cout << "obs_dim: " << obs_dim_ << ", action_dim: " << act_dim_ << "\n";
}

void Lite3FlatPolicyRunnerONNX::OnEnter() {
    run_cnt_ = 0;
    current_obs_.setZero(obs_dim_);
    std::cout << "[ONNX ENTER] PolicyRunner entered: " << policy_name_ << std::endl;
}

RobotAction Lite3FlatPolicyRunnerONNX::GetRobotAction(const RobotBasicState& ro) {
...
// Private helpers
// ---------------------------------------------------------------------------
std::vector<int> Lite3FlatPolicyRunnerONNX::generate_permutation(
