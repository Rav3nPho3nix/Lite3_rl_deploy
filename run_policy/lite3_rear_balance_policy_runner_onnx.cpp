// Policy pour l'equilibre sur les pattes arriere
...
// ---------------------------------------------------------------------------
struct Lite3RearBalancePolicyRunnerONNX::OrtImpl {
    Ort::Env env;
...
// ---------------------------------------------------------------------------
Lite3RearBalancePolicyRunnerONNX::Lite3RearBalancePolicyRunnerONNX(std::string policy_name)
    ...
    model_path_ = GetAbsPath() + "/../policy/ppo/lite3_rear_balance_policy.onnx";
...
Lite3RearBalancePolicyRunnerONNX::~Lite3RearBalancePolicyRunnerONNX() = default;

// ---------------------------------------------------------------------------
// Public interface
// ---------------------------------------------------------------------------
void Lite3RearBalancePolicyRunnerONNX::DisplayPolicyInfo() {
    std::cout << "ONNX policy: " << policy_name_ << "\n";
    std::cout << "path: " << model_path_ << "\n";
    std::cout << "obs_dim: " << obs_dim_ << ", action_dim: " << act_dim_ << "\n";
}

void Lite3RearBalancePolicyRunnerONNX::OnEnter() {
    run_cnt_ = 0;
    current_obs_.setZero(obs_dim_);
    std::cout << "[ONNX ENTER] PolicyRunner entered: " << policy_name_ << std::endl;
}

RobotAction Lite3RearBalancePolicyRunnerONNX::GetRobotAction(const RobotBasicState& ro) {
...
// Private helpers
// ---------------------------------------------------------------------------
std::vector<int> Lite3RearBalancePolicyRunnerONNX::generate_permutation(
